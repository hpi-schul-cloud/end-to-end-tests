...
Feature: Create, edit and delete a topic in the course on the HPI SchulCloud page
	...
	Scenario Outline: <userRole> creates a new topic in course
		...
		Then <userRole> should see that created topic with name <topicName> is shown on the topics list
		When <userRole> clicks on the topic with name <topicName>
		Then <userRole> should see that the topic with name <topicName> is visible one the topic page
		Examples:
