...
Feature: Test set for user settings
	...
	Scenario Outline: As a user, I want to be able to change the language
		...
		Examples:
			...
			| admin    | olivier.admin.qa@schul-cloud.org  | Schulcloud1qa! | Ukrainian | ПАНЕЛЬ КЕРУВАННЯ,КУРСИ,КОМАНДИ,МОЇ ФАЙЛИ,НОВИНИ,КАЛЕНДАР,НАВЧАЛЬНИЙ МАГАЗИН,УПРАВЛІННЯ,ДОВІДКОВИЙ РОЗДІЛ          |
			| teacher  | dmitri.teacher.qa@schul-cloud.org | Schulcloud1qa! | Ukrainian | ПАНЕЛЬ КЕРУВАННЯ,КУРСИ,КОМАНДИ,ЗАВДАННЯ,МОЇ ФАЙЛИ,НОВИНИ,КАЛЕНДАР,НАВЧАЛЬНИЙ МАГАЗИН,УПРАВЛІННЯ,ДОВІДКОВИЙ РОЗДІЛ |
