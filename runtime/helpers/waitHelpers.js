...
async function waitAndSetValue(selectorOrElement, expectedValue, timeout = MEDIUM_WAIT_MILLIS) {
	...
	const msg = "Could not set value: " + expectedValue + ' for element: "' + element.selector + '" within time: ' + timeout;
	let actualValue = "";
	try {
		await driver.waitUntil(
			async () => {
...
async function waitUntilElementAttributeEquals(selectorOrElement, attributeName, expectedValue , timeout = setValueTimeout) {
	const element = await waitUntilElementIsPresent(selectorOrElement);
...
async function waitUntilElementAttributeContains(selectorOrElement, attributeName, expectedValue , timeout = setValueTimeout) {
	const element = await waitUntilElementIsPresent(selectorOrElement);
